...
try {
  ...
  console.log(`📊 Loaded ${jsonData.charts?.length || 0} candles and ${jsonData.traderLogOperations?.length || 0} operations`);
} catch (error) {
...
wss.on('connection', (ws) => {
  ...
  const interval = setInterval(() => {
    ...
    if (ws.readyState === ws.OPEN) {
      ...
      const operations = jsonData.traderLogOperations.filter(op => op.time === candle.time);
      ...
      console.log(`📊 Sent candle #${currentIndex + 1}/${jsonData.charts.length} (${new Date(candle.time).toISOString()})`);
      currentIndex++;
    }
